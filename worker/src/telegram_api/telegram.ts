...
const COMMANDS = [
    ...
    {
        command: "new",
        description: "Create a new email address. To customize the email address, enter /new <name>@<domain>, name [a-z0-9] valid"
    },
    {
        command: "address",
    ...
    {
        command: "bind",
        description: "Bind email address, enter /bind <email address token>"
    },
    {
        command: "unbind",
        description: "Unbind email address, enter /unbind <email address>"
    },
    {
        command: "delete",
        description: "Delete email address, enter /delete <email address>"
    },
    {
        command: "mails",
        description: "View emails, enter /mails <email address>, if no address is entered, the first address will be viewed by default"
    },
    {
        command: "cleaninvalidadress",
        description: "Clean invalid addresses, enter /cleaninvalidadress"
    },
]
...
export function newTelegramBot(c: Context<HonoCustomType>, token: string): Telegraf {
    ...
    bot.command("start", async (ctx: TgContext) => {
        ...
        return await ctx.reply(
            "Welcome to use this bot, you can open the mini app \n\n"
    ...
    bot.command("bind", async (ctx: TgContext) => {
        ...
        try {
            ...
            if (!jwt) {
                return await ctx.reply("Please enter the token");
            }
    ...
    bot.command("address", async (ctx) => {
        ...
        try {
            ...
            return await ctx.reply(`Failed to retrieve address list: ${(e as Error).message}`);
        }
    });
    ...
    bot.command("cleaninvalidadress", async (ctx: TgContext) => {
        ...
        try {
            ...
            return await ctx.reply(`Successfully cleaned invalid addresses:\n\n`
    ...
    bot.on(callbackQuery("data"), async ctx => {
        ...
        try {
            ...
            console.log(`Failed to retrieve email: ${(e as Error).message}`, e);
            return await ctx.answerCbQuery(`Failed to retrieve email: ${(e as Error).message}`);
        }
...
const parseMail = async (
    ...
    try {
        ...
        if (parsedText.length && parsedText.length > 1000) {
            parsedText = parsedEmail?.text.substring(0, 1000) + "\n\n...\nMessage too long, please check in the mini app";
        }
        return {
            isHtml: false,
            mail: `From: ${parsedEmail?.sender || "No sender"}\n`
                ...
                + `Content:\n${parsedText || "Parsing failed, please check in the mini app"}`
        };
    } catch (e) {
        return {
            isHtml: false,
